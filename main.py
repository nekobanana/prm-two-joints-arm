...
def cartesian_to_config_space(x, y, l1, l2):
    if x > 0:
        config1, config2 = cartesian_to_config_space_x_pos(x, y, l1, l2)
    else:
        config1, config2 = cartesian_to_config_space_x_pos(-x, y, l1, l2)
...
def cartesian_to_config_space_x_pos(x, y, l1, l2):
    ...
    theta1_neg = np.arctan(y / x) + np.arctan((l2 * np.sin(theta2_pos)) / (l1 + l2 * np.cos(theta2_pos))) # non dovrebbe essere theta2_neg in questa formula?
    return CPoint(theta1_pos, theta2_pos), CPoint(theta1_neg, theta2_neg)

def mod2pi(c: CPoint):
    c.point = np.mod(c.point, 2*np.pi)
    return c
def plot_arm(ax, alpha, l1, x, y):
    x1, y1 = l1 * np.cos(alpha), l1 * np.sin(alpha)
    ax.plot([0, x1], [0, y1], linewidth=3)
    ax.plot([x1, x], [y1, y], linewidth=3)

def plot_obs_in_cart_space(ax, points):
    points.append(points[0])
    x, y = zip(*points)
    ax.plot(x, y)

def plot_in_c_space(ax, points: list[CPoint], obstacle=True):
    if obstacle:
        points.append(points[0])
...
def main():
    ...
    obstacle1_cartesian = [(3, 4), (6, 5), (5, 7)]

    if not check_arm_reach(x, y, l1, l2):
        print("Point is outside arm length")
        return
    config1, config2 = cartesian_to_config_space(x, y, l1, l2)
    plot_arm(ax_w, config1.theta1, l1, x, y)
    plot_arm(ax_w, config2.theta1, l1, x, y)
    plot_obs_in_cart_space(ax_w, obstacle1_cartesian)
    ...
    plot_in_c_space(ax_c, [config1, config2], obstacle=False)
